...
offboard_class::offboard_class(ros::NodeHandle* nodehandle, DBSCAN::DBSCAN ds):nh(*nodehandle), ds_home(ds)
{
	...
	ned_euler.x = 0, ned_euler.y = 0, ned_euler.z = 0;
	// 创建一个参数动态配置的服务器
    static dynamic_reconfigure::Server<CADC::pid_planeConfig> server;
    
    // 定义回调函数
    static dynamic_reconfigure::Server<CADC::pid_planeConfig>::CallbackType PID_cb;
	PID_cb = boost::bind(&offboard_class::Plane_PID_cb, this, _1);
	server.setCallback(PID_cb);
}
...
// 动态调参
void offboard_class::Plane_PID_cb(CADC::pid_planeConfig &config)
{
	KP_X_P = config.KP_X_P;
	KI_X_P = config.KI_X_P;
	KD_X_P = config.KD_X_P;

	KP_Y_P = config.KP_Y_P;
	KI_Y_P = config.KI_Y_P;
	KD_Y_P = config.KD_Y_P;
	
	MaxSpeedPlane_X = config.MaxSpeedPlane_X;
	MaxSpeedPlane_Y = config.MaxSpeedPlane_Y;
	ROS_INFO_STREAM("the Plane X P I D MaxSpeedPlane_X : " << KP_X_P << ', ' << KI_X_P << ', ' << KD_X_P << ', X SPEED ' << MaxSpeed_X);
	ROS_INFO_STREAM("the Plane Y P I D MaxSpeedPlane_X : " << KP_Y_P << ', ' << KI_Y_P << ', ' << KD_Y_P << ', Y SPEED ' << MaxSpeed_Y);
}
